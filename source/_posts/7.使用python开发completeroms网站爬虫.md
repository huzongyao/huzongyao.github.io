...
def start_fetch():
    max_page = 65
    for i in range(32, max_page):
        url = 'http://www.completeroms.com/roms/nintendo/%d' % i
        req = requests.get(url)
        bs = BeautifulSoup(req.text, features='html5lib')
        tr_nodes = bs.find('table', class_='table table-striped table-bordered table-hover').find_all('tr')
        pool = ThreadPool(1)
        pool_requests = makeRequests(get_game_in_pool, tr_nodes, None, None)
        [pool.putRequest(req) for req in pool_requests]
        pool.wait()
...
def get_game_in_pool(tr_node):
    ...
    if a_node is None:
        ...
        return
    a_url = a_node.attrs['href']
    a_text = a_node.text.strip()
    deal_with_one_game(a_url, a_text)


def deal_with_one_game(url, a_text):
    rom_root = 'roms/'
    img_root = 'imgs/'
    ...
    last_id = url.rfind('/') + 1
    rom_id = url[last_id:]
    url = 'http://www.completeroms.com/thankyou.php?id=%s' % rom_id
    req = requests.get(url)
    bs = BeautifulSoup(req.text, features='html5lib')
    ...
    if img_node:
        ...
        ptn = re.compile(r'.*\.(?P<ext_name>.*)')
        m = ptn.search(img_url)
        ext_name = '.jpg'
        if hasattr(m, 'group'):
            ext_name = '.' + m.group('ext_name').lower()
        ptn = re.compile(r'(?P<simple_name>.*)\.zip')
        m = ptn.search(file_name)
        if hasattr(m, 'group'):
            img_name = m.group('simple_name') + ext_name
...
```
做的比较简单，如果需要稳定的话还要再加些空判断和异常处理，校验之类。我的目的只是几千个ROM一夜之间下载到手，其他的就不用管了，到此完。